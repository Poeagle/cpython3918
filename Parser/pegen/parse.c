...
#define ale_bitwise_or_type 1093
...
#define _tmp_92_type 1252
#define _loop0_94_type 1253
#define _gather_93_type 1254
#define _tmp_95_type 1255
#define _tmp_96_type 1256
#define _tmp_97_type 1257
#define _tmp_98_type 1258
#define _loop1_99_type 1259
#define _tmp_100_type 1260
#define _tmp_101_type 1261
#define _loop0_103_type 1262
#define _gather_102_type 1263
#define _loop1_104_type 1264
#define _loop0_105_type 1265
#define _loop0_106_type 1266
#define _loop0_108_type 1267
#define _gather_107_type 1268
...
static asdl_seq *_loop1_121_rule(Parser *p);
...
small_stmt_rule(Parser *p)
{
    ...
    { // ('pass' | 'go')
        ...
        D(fprintf(stderr, "%*c> small_stmt[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "('pass' | 'go')"));
        void *_tmp_15_var;
        if (
            (_tmp_15_var = _tmp_15_rule(p))  // 'pass' | 'go'
        )
        {
            D(fprintf(stderr, "%*c+ small_stmt[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "('pass' | 'go')"));
            Token *_token = _PyPegen_get_last_nonnwhitespace_token(p);
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s small_stmt[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "('pass' | 'go')"));
    }
    { // &'del' del_stmt
        if (p->error_indicator) {
            p->level--;
...
assignment_rule(Parser *p)
{
    ...
    { // NAME ':' expression ['=' annotated_rhs]
        ...
        if (
            ...
            (b = expression_rule(p))  // expression
            &&
            (c = _tmp_20_rule(p), !p->error_indicator)  // ['=' annotated_rhs]
        )
        {
    ...
    { // ('(' single_target ')' | single_subscript_attribute_target) ':' expression ['=' annotated_rhs]
        ...
        if (
            ...
            (b = expression_rule(p))  // expression
            &&
            (c = _tmp_22_rule(p), !p->error_indicator)  // ['=' annotated_rhs]
        )
        {
            D(fprintf(stderr, "%*c+ assignment[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "('(' single_target ')' | single_subscript_attribute_target) ':' expression ['=' annotated_rhs]"));
            Token *_token = _PyPegen_get_last_nonnwhitespace_token(p);
...
assert_stmt_rule(Parser *p)
{
    ...
    { // 'assert' expression [',' expression]
        ...
        if (
            ...
            (a = expression_rule(p))  // expression
            &&
            (b = _tmp_30_rule(p), !p->error_indicator)  // [',' expression]
        )
        {
...
import_from_as_name_rule(Parser *p)
{
    ...
    { // NAME ['as' NAME]
        ...
        if (
            (a = _PyPegen_name_token(p))  // NAME
            &&
            (b = _tmp_36_rule(p), !p->error_indicator)  // ['as' NAME]
        )
        {
...
dotted_as_name_rule(Parser *p)
{
    ...
    { // dotted_name ['as' NAME]
        ...
        if (
            ...
            &&
            (b = _tmp_39_rule(p), !p->error_indicator)  // ['as' NAME]
        )
        {
...
except_block_rule(Parser *p)
{
    ...
    { // 'except' expression ['as' NAME] ':' block
        ...
        if (
            ...
            (e = expression_rule(p))  // expression
            &&
            (t = _tmp_50_rule(p), !p->error_indicator)  // ['as' NAME]
            &&
            (_literal = _PyPegen_expect_token(p, 11))  // token=':'
            &&
            (b = block_rule(p))  // block
...
raise_stmt_rule(Parser *p)
{
    ...
    { // 'raise' expression ['from' expression]
        ...
        if (
            ...
            (a = expression_rule(p))  // expression
            &&
            (b = _tmp_51_rule(p), !p->error_indicator)  // ['from' expression]
        )
        {
...
function_def_raw_rule(Parser *p)
{
    ...
    { // ASYNC 'def' NAME '(' params? ')' ['->' expression] ':' func_type_comment? block
        ...
        if (
            ...
            (_literal_1 = _PyPegen_expect_token(p, 8))  // token=')'
            &&
            (a = _tmp_53_rule(p), !p->error_indicator)  // ['->' expression]
            &&
            (_literal_2 = _PyPegen_expect_token(p, 11))  // token=':'
            &&
            (tc = func_type_comment_rule(p), !p->error_indicator)  // func_type_comment?
...
class_def_raw_rule(Parser *p)
{
    ...
    { // 'class' NAME ['(' arguments? ')'] ':' block
        ...
        if (
            ...
            (a = _PyPegen_name_token(p))  // NAME
            &&
            (b = _tmp_70_rule(p), !p->error_indicator)  // ['(' arguments? ')']
            &&
            (_literal = _PyPegen_expect_token(p, 11))  // token=':'
            &&
...
slice_rule(Parser *p)
{
    ...
    { // expression? ':' expression? [':' expression?]
        ...
        if (
            ...
            &&
            (c = _tmp_95_rule(p), !p->error_indicator)  // [':' expression?]
        )
        {
...
tuple_rule(Parser *p)
{
    ...
    { // '(' [star_named_expression ',' star_named_expressions?] ')'
        ...
        if (
            (_literal = _PyPegen_expect_token(p, 7))  // token='('
            &&
            (a = _tmp_100_rule(p), !p->error_indicator)  // [star_named_expression ',' star_named_expressions?]
            &&
            (_literal_1 = _PyPegen_expect_token(p, 8))  // token=')'
        )
        {
...
args_rule(Parser *p)
{
    ...
    { // ','.(starred_expression | named_expression !'=')+ [',' kwargs]
        ...
        void *b;
        if (
            (a = _gather_107_rule(p))  // ','.(starred_expression | named_expression !'=')+
            &&
            (b = _tmp_109_rule(p), !p->error_indicator)  // [',' kwargs]
        )
        {
...
star_targets_tuple_seq_rule(Parser *p)
{
    ...
    { // star_target ((',' star_target))+ ','?
        ...
        if (
            (a = star_target_rule(p))  // star_target
            &&
            (b = _loop1_121_rule(p))  // ((',' star_target))+
            &&
            (_opt_var = _PyPegen_expect_token(p, 12), !p->error_indicator)  // ','?
        )
        {
...
invalid_arguments_rule(Parser *p)
{
    ...
    { // expression for_if_clauses ',' [args | expression for_if_clauses]
        ...
        if (
            ...
            (_literal = _PyPegen_expect_token(p, 12))  // token=','
            &&
            (_opt_var = _tmp_125_rule(p), !p->error_indicator)  // [args | expression for_if_clauses]
        )
        {
            D(fprintf(stderr, "%*c+ invalid_arguments[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "expression for_if_clauses ',' [args | expression for_if_clauses]"));
...
invalid_kwarg_rule(Parser *p)
{
    ...
    { // !(NAME '=') expression '='
        ...
        Token * b;
        if (
            _PyPegen_lookahead(0, _tmp_126_rule, p)
            &&
            (a = expression_rule(p))  // expression
            &&
...
_tmp_15_rule(Parser *p)
{
    ...
    { // 'go'
        ...
        D(fprintf(stderr, "%*c> _tmp_15[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "'go'"));
        Token * _keyword;
        if (
            (_keyword = _PyPegen_expect_token(p, 532))  // token='go'
        )
        {
            D(fprintf(stderr, "%*c+ _tmp_15[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "'go'"));
            _res = _keyword;
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s _tmp_15[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "'go'"));
    }
    _res = NULL;
  done:
    p->level--;
...
_gather_107_rule(Parser *p)
{
    ...
    asdl_seq * _res = NULL;
    int _mark = p->mark;
    { // (starred_expression | named_expression !'=') _loop0_108
        if (p->error_indicator) {
            p->level--;
            return NULL;
        }
        D(fprintf(stderr, "%*c> _gather_107[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "(starred_expression | named_expression !'=') _loop0_108"));
        void *elem;
        asdl_seq * seq;
        if (
            (elem = _tmp_148_rule(p))  // starred_expression | named_expression !'='
        ...
        D(fprintf(stderr, "%*c%s _gather_107[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "(starred_expression | named_expression !'=') _loop0_108"));
    }
    _res = NULL;
  done:
    p->level--;
...
_loop1_121_rule(Parser *p)
{
    ...
    { // (',' star_target)
        ...
        D(fprintf(stderr, "%*c> _loop1_121[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "(',' star_target)"));
        void *_tmp_150_var;
        while (
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s _loop1_121[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "(',' star_target)"));
    }
    if (_n == 0 || p->error_indicator) {
        PyMem_Free(_children);
    ...
    for (int i = 0; i < _n; i++) asdl_seq_SET(_seq, i, _children[i]);
    PyMem_Free(_children);
    _PyPegen_insert_memo(p, _start_mark, _loop1_121_type, _seq);
    p->level--;
    return _seq;
}
