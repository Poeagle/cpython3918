                 ...
                 freelist_t *freelist)
    {
        ...
        if (!PySequence_Check(arg) || PyBytes_Check(arg))
        {
            ...
            PyOS_snprintf(msgbuf, bufsize,
                          toplevel ? "expected %d arguments, not %.50s" : "must be %d-item sequence, not %.50s",
                          n,
...
#define RETURN_ERR_OCCURRED return msgbuf
        ...
        case 'z': /* text string, bytes-like object or None */
        {
            ...
            { /* a string or read-only bytes-like object */
                /* "s#" or "z#" */
        ...
        case 'w':
        { /* "w*": memory buffer, read-write access */
...
#define IS_END_OF_FORMAT(c) (c == '\0' || c == ';' || c == ':')
                     ...
                     char **kwlist, va_list *p_va, int flags)
    {
        ...
        if (nargs + nkwargs > len)
        {
            ...
            PyErr_Format(PyExc_TypeError,
                         "%.200s%s takes at most %d %sargument%s (%zd given)(cym)",
                         (fname == NULL) ? "function" : fname,
                         (fname == NULL) ? "" : "()",
                         len,
                         (nargs == 0) ? "keyword " : "",
                         ...
                         struct _PyArg_Parser *parser, va_list *p_va, int flags)
    {
        ...
        if (args == NULL || !PyTuple_Check(args) || (keywords != NULL && !PyDict_Check(keywords)))
        {
            PyErr_BadInternalCall();
            return 0;
        }
